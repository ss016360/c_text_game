...
void pickUpItem(player* player, room* currentRoom, int choice) {
	...
	// Check if the room has an item at the specified choice
		...
		// Copy the item from itemList based on the user's choice
                player->inventory[player->inventorySize] = malloc(strlen(itemList[choice - 1]) + 1);
                strcpy(player->inventory[player->inventorySize], itemList[choice - 1]);
                printf("Picked up %s\n", itemList[choice - 1]);
                
		// Update the room's item to NULL after picking it up
		free(currentRoom->items);
...
int getMenuChoice(game* game) {
  ...
  if (choice == 4) {
    ...
    if (itemChoice >= 0 && itemChoice < 5) {
      ...
      pickUpItem(game->player, game->player->currentRoom, chosenIndices[itemChoice]);
    } else {
      printf("Invalid choice!\n");
    }
  }
