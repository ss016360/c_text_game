...
// we need to keep track of the number of rooms
...
void pickUpItem(player* player, room* currentRoom, int choice) {
  if (currentRoom->items != NULL) {
    ...
    if (player->inventorySize < MAX_INVENTORY_SIZE) {
      ...
      printf("Picked up %s\n", currentRoom->items);
      currentRoom->items = NULL; // Remove the item from the room after picking it up
    } else {
...
room* loadRooms() {
  ...
  //here is a list of items that could be in the rooms
  char* itemList[] = {"sword", "shield", "flower", "book", "potion", "wand", "ring", "poison", "key", "looking-glass"};
  ...
  for (int i = 0; i < numLines; i++) {
    ...
    rooms[i] = room;
     ...
     // Assign items randomly to rooms
     for (int i = 0; i < numLines; i++) {
     int itemIndex = rand() % 10; // Randomly select an item from the list
     rooms[i].items = itemList[itemIndex]; // Assign the item to the room
  }
