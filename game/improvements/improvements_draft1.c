...
/*   
  // I need to read information from connections.csv
  // I need to create a 2D array to store the information
  ...
  int numLines2 = 0;
  char line2[500];
  while (fgets(line2, 500, file2) != NULL) {
    numLines2++;
  }
  // we need to rewind the file
  rewind(file2);
  
  //I need to read the number values from the file2 file
  //I need to read all of the lines from the file
  //these need to me used with the connection.struct
  

  // we need to allocate memory for the connections
  connection* connections = malloc(sizeof(connection) * numLines2);
  
  // we need to read the connections from the file
  // we need to read each line using a for loop
  // we need to have a loop to read each line of the connections.csv file
  // we need to enter each row as a list into the 2D array
  ...
  for (int i = 0; i < numLines2; i++) {
    ...
    //we need to copy the string into the room name
    connection.name = malloc(sizeof(char) * strlen(name) + 1);
    strcpy(connection.name, name);
    //we need to copy the string into the room description
    connection.description = malloc(sizeof(char) * strlen(description) + 1);
    strcpy(connection.description, description);
